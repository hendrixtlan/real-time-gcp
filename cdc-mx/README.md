...
# 4. Deploy completo
./scripts/deploy.sh init      # crea bucket de tfstate y secrets
./scripts/deploy.sh build     # construye imágenes Docker
./scripts/deploy.sh apply     # terraform apply
./scripts/deploy.sh schema    # aplica schemas SQL en AlloyDB y BQ
./scripts/deploy.sh verify    # health checks
```

Detalle de cada paso en `docs/RUNBOOK.md`.
